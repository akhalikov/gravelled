...
It is data, not software: no build, no tests, no CI. Rider is based in Kraków; main bike is a
Canyon Grizl CF 8 ESC.
...
- **Files:** lowercase-hyphenated `.md` at the repo root. No subdirectories for data.
  `.claude/workspace/` is git-ignored scratch.
...
- Ride data is added **by hand** — Garmin auto-append is not built (see `garmin-mcp.md`).
